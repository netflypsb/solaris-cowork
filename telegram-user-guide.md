...
**Fix**: Ensure bot is running. The bot username is auto-detected at startup and shown in Settings.
...
1. **Protect Your Bot Token**
   - Never share your bot token in public channels
   - If compromised, use `/revoke` in BotFather immediately
   - Store tokens in the Solaris app, not in code repositories

2. **Use Appropriate Access Modes**
   - **Whitelist** for personal/team use (most secure)
   - **Approval** for vetted communities
   - **Open** only for public SaaS with monitoring

3. **Monitor User Activity**
   - Run `/users` regularly to check for unexpected users
   - Check `/admin` analytics for unusual patterns
   - Set up alerts for high-volume usage

4. **Secure Invite Links**
   - Use low usage limits: `/invite 5 basic` not `/invite 1000 basic`
   - Share links through secure channels (encrypted messaging)
   - Revoke links by blocking users who shouldn't have access

5. **Group Mode Security**
   - Only add the bot to trusted groups
   - Use `mention` or `command` activation (not `all`) in large groups
   - Remove the bot immediately if the group is compromised

6. **Payment Security**
   - Never share Stripe Secret Keys with users
   - Use test mode for development
   - Verify webhook signatures if implementing custom handlers

7. **Data Protection**
   - Don't store sensitive credentials in project knowledge files
   - Use separate Solaris projects for different trust levels
   - Regularly audit what files are in your project directory

### For Users

1. **Verify Bot Authenticity**
   - Check the bot username matches what the owner provided
   - Look for the "Bot" label in Telegram
   - Be cautious of bots asking for external credentials

2. **Protect Your Privacy**
   - Don't share personal information with bots unless necessary
   - Remember that bot owners may see your Telegram username
   - Use the `/cancel` command to stop any concerning interactions
...
- [ ] Configured bot commands menu
- [ ] Set privacy mode (DISABLED for groups)
- [ ] Tested bot token in Solaris
- [ ] Generated and used pairing code
- [ ] Verified bot responds to messages
- [ ] Tested file delivery (send and receive)
- [ ] Set appropriate access mode
- [ ] Configured per-user limits (for teams/groups)
- [ ] Set up payment methods (if monetized)
- [ ] Created subscription plans
- [ ] Written custom welcome message
- [ ] Tested deep links (invite and plan)
- [ ] Added knowledge base files (if needed)
- [ ] Documented bot purpose for users

### Team/Group Launch Checklist

- [ ] Selected appropriate access mode
- [ ] Set per-user message limits
- [ ] Configured max concurrent AI
- [ ] Decided on pairing vs invite onboarding
- [ ] Prepared user onboarding instructions
- [ ] Set up user monitoring workflow
- [ ] Planned broadcast message schedule

### SaaS Launch Checklist

- [ ] Stripe account created and verified
- [ ] Stripe Provider Token configured
- [ ] Payment test completed (test mode)
- [ ] Subscription plans finalized
- [ ] Pricing strategy validated
- [ ] Free tier limits set appropriately
- [ ] Auto-approve on payment enabled
- [ ] Welcome message includes value proposition
- [ ] Marketing deep links generated
...
1. **Set a compelling free tier** — Enough to demonstrate value, not enough to satisfy ongoing needs
2. **Use Stars for impulse purchases** — Lower friction than Stripe for small amounts
3. **Price anchoring** — Make the Pro plan look like the best value
4. **Broadcast updates** — Use `/broadcast` to re-engage inactive users
5. **Plan deep links** — Share `?start=plan_pro` on social media for direct conversions
6. **Custom welcome message** — First impressions matter, write a compelling welcome

### Optimizing User Experience

1. **Enable both indicators** — Typing + reactions give users confidence
2. **Set appropriate concurrent limits** — Too low = queue frustration, too high = slow responses
3. **Use custom knowledge** — Domain-specific knowledge makes responses dramatically better
4. **Test your bot** — Send real questions and improve knowledge files based on gaps
5. **Quick response to questions** — When the agent asks a question, answer quickly from Telegram

### Security Best Practices

1. **Protect your Stripe keys** — Never share Provider Tokens publicly
2. **Use Whitelist for sensitive data** — Don't use Open mode if your project has secrets
3. **Monitor for abuse** — Check `/users` regularly, block bad actors
4. **Separate projects** — Use different Solaris projects for different bots/audiences
5. **Regular updates** — Keep knowledge files current and accurate
...
| Term | Definition |
|------|-----------|
| **Bot Owner** | The first user who pairs with the bot. Has unlimited access and admin commands. |
| **Pairing Code** | A 6-digit code used to link a Telegram account to the bot. |
| **Deep Link** | A URL with parameters that triggers specific bot actions on click. |
| **Invite Code** | A code embedded in a deep link that auto-registers users. |
| **Stars (XTR)** | Telegram's digital currency for in-app purchases. |
| **Plan** | A subscription tier defining price, message limits, and billing period. |
| **Access Mode** | Controls how new users can join: Whitelist, Open, or Approval. |
| **Processing Indicator** | Visual feedback (typing, reactions) shown while AI processes messages. |
| **Group Mode** | Special mode where the bot lives in a Telegram group chat. |
| **Auto-Approve on Payment** | Setting that automatically grants access after successful payment. |

---

*This guide covers Solaris Telegram Integration v1.0.1. Features may evolve with future updates.*
